...
class PHPPagingmini {
	...
	var $mostrarPrimera = "<span style='width:18px; background-color:#D3D3D3;'><font face=arial><b>&nbsp;&nbsp;Primera&nbsp;&nbsp;</b></font></span>";
 
	var $mostrarUltima = "<span style='width:18px; background-color:#D3D3D3;'><font face=arial><b>&nbsp;&nbsp;&Uacute;ltima&nbsp;&nbsp;</b></font></span>";
 
	var $mostrarAnterior = "<span style='width:18px; background-color:#D3D3D3;'><font face=arial><b>&nbsp;&nbsp;&lt;&nbsp;&nbsp;</b></font></span>";
 
	var $mostrarSiguiente = "<span style='width:18px; background-color:#D3D3D3;'><font face=arial><b>&nbsp;&nbsp;&gt;&nbsp;&nbsp;</b></font></span>";
 
	var $mostrarAdyacentes = "<span style='width:18px; background-color:#D3D3D3;'><font face=arial><b>&nbsp;&nbsp;{n}&nbsp;&nbsp;</b></font></span>";
 
	var $mostrarActual = "<span style='width:18px; background-color:#BDBDBD;'><font face=arial><b>&nbsp;&nbsp;{n}&nbsp;&nbsp;</b></font></span>";
	...
	function ejecutar () {
		...
		if($this->numTotalRegistros >= 1) {
			...
			if(!empty($this->sql)) {
				...
				if(!$result) return $this->error(false, @mysql_error(), $sql, ereg("limit[ ]+[0-9]+(,[ ]*[0-9]+)?", strtolower($sql)));
	...
	function error($desarrollo = true, $msg, $query = false, $limit_error = null) {
		if($this->modo != 'publicacion' and $this->mostre_error == false) {
			if($this->modo == 'reporte') {
				...
				if($limit_error == true) $error.= "El error probablemente se deba a que en la consulta MySQL que usted indicó aparentemente ya existía una cláusula LIMIT, la cuál es añadida automáticamente por el paginador.<br /><br />";
				$error.= "Si no logra solucionar el problema, envíe un mensaje a Ioseet.com, indicando el error mostrado y la consulta que generó el error";
			} else {
				$error = "Hubo un error ejecutando la paginación. El mensaje devuelto es: <blockquote><code style='color: #080; font-size: 13px;'><strong>$msg</strong></code></blockquote>Si no logra solucionar el problema, envíe un mensaje a Ioseet.com indicando el error mostrado y la consulta que generó el error";
			}
			$p = "<div style='border: 1px solid #666; background-color: #f6f6f6; margin: 5px 10px; padding: 10px 5px;'>";
			$p.= "<div style='color: #f00; font: 18px Georgia; margin: 0 5px; border-bottom: 1px dotted #f00;'><strong>Error</strong></div>";
			$p.= "<div style='font: 12px Verdana; margin: 15px 19px;'>$error</div>";
			$p.= "<div style='font: 12px Georgia; text-align: right; color: #777; margin: 0; padding: 1px 4px;'>Paginacion</div></div>";
			echo $p;
		} else {
			$this->error = $msg;
		}
