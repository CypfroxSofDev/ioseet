<?php
$UploadDirectory	= '../misc/fondoperfil/'; //Upload Directory, ends with slash & make sure folder exist
...
{	
	...
	$NewFileName = $NewFileName.'_'.$RandNumber.$ImageExt;
		...
		//connect & insert file record in database
					...
					@mysql_query("UPDATE `DisenoFD` SET `fondo`='$Dir' WHERE `idEmpresa`='".$user['idEmpresa']."'");
               ...
               $gn = mysql_query("SELECT * FROM `Empresa` WHERE `idEmpresa`='".$_SESSION['idEmpresa']."'") or die(mysql_error());
		        ...
		        <script  language="javascript">window.location="../'.$n['nick'].'"</script>
                </div></div>';
