<?php
$UploadDirectory	= '../misc/fotosperfil/'; //Upload Directory, ends with slash & make sure folder exist
...
{	
    ...
    $Dir = $NewFileName;
		...
		//connect & insert file record in database
				...
				@mysql_query("UPDATE `DisenoFD` SET `foto`='$Dir' WHERE `idEmpresa`='".$user['idEmpresa']."'");
		
               $gn = mysql_query("SELECT * FROM `DisenoFD` WHERE `idEmpresa`='".$_SESSION['idEmpresa']."'") or die(mysql_error());
	                        $n = mysql_fetch_array($gn);	
                echo"<div id=\"bordep\"><img src=\"misc/fotosperfil/".$n['foto']."\" style=\"width: 120px; height:120px; border-radius: 3px; \" alt=\"\"/></div>"; 
